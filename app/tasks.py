...
def map_source(data):
    ...
    def check_article(article_info):
        ...
        cur.execute(f"SELECT count(*) as cnt FROM articles WHERE id={new_id} AND updated='{new_updated}'::timestamp;")
        result = cur.fetchone()
        cur.close()
...
def map_article(data):
    ...
    def check_is_covid_related(article_data):
        ...
        for check in ["title_header", "title", "lead"]:
            ...
            if check_text.startswith("corona") or " corona" in check_text or "covid-19" in check_text:
                return True
        return False
    ...
    def insert_or_update_article(article_data):
        ...
        cur = db_conn.cursor()
        sql_text = generate_sql_upsert("public", "articles", article_data.keys(), ["id"])
